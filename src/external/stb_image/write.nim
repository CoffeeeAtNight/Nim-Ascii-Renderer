...
proc writePNG*(filename: string; w, h, comp: int; data: openarray[byte];
    stride_in_bytes: int = 0): bool {.discardable.} =
  return (stbi_write_png(filename.cstring, w.cint, h.cint, comp.cint, data[
      0].unsafeAddr, stride_in_bytes) == 1)
...
##
## Please see the documentation in the `stbi_image_write.h` file for more info.
proc writeBMP*(filename: string; w, h, comp: int; data: openarray[
    byte]): bool {.discardable.} =
  return (stbi_write_bmp(filename.cstring, w.cint, h.cint, comp.cint, data[
      0].unsafeAddr) == 1)
...
proc writeTGA*(filename: string; w, h, comp: int; data: openarray[byte];
    useRLE: bool = true): bool {.discardable.} =
  # Set RLE option
  stbi_write_tga_with_rle = if useRLE: 1 else: 0
  return (stbi_write_tga(filename.cstring, w.cint, h.cint, comp.cint, data[
      0].unsafeAddr) == 1)
...
##
## Please see the documentation in the `stbi_image_write.h` file for more info.
proc writeHDR*(filename: string; w, h, comp: int; data: openarray[
    float32]): bool {.discardable.} =
  return (stbi_write_hdr(filename.cstring, w.cint, h.cint, comp.cint, data[
      0].unsafeAddr) == 1)
...
##
## Please see the documentation in the `stbi_image_write.h` file for more info.
proc writeJPG*(filename: string; w, h, comp: int; data: openarray[byte];
    quality: int): bool {.discardable.} =
  return (stbi_write_jpg(filename.cstring, w.cint, h.cint, comp.cint, data[
      0].unsafeAddr, quality.cint) == 1)
...
##
## By default the stride is set to zero.
proc writePNG*(w, h, comp: int; data: openarray[byte];
    stride_in_bytes: int = 0): seq[byte] =
  var buffer = newStringStream()

  if stbi_write_png_to_func(streamWriteData, buffer.addr, w.cint, h.cint,
      comp.cint, data[0].unsafeAddr, stride_in_bytes.cint) != 1:
...
proc writeBMP*(w, h, comp: int; data: openarray[byte]): seq[byte] =
  ...
  if stbi_write_bmp_to_func(streamWriteData, buffer.addr, w.cint, h.cint,
      comp.cint, data[0].unsafeAddr) != 1:
...
## By default this function will save the TGA with run-length encoding, but this
## can be turned off by setting `useRLE` to `false`.
proc writeTGA*(w, h, comp: int; data: openarray[byte];
    useRLE: bool = true): seq[byte] =
  # Set RLE option
  stbi_write_tga_with_rle = if useRLE: 1 else: 0
  var buffer = newStringStream()

  if stbi_write_tga_to_func(streamWriteData, buffer.addr, w.cint, h.cint,
      comp.cint, data[0].unsafeAddr) != 1:
...
proc writeHDR*(w, h, comp: int; data: openarray[byte]): seq[byte] =
  ...
  if stbi_write_hdr_to_func(streamWriteData, buffer.addr, w.cint, h.cint,
      comp.cint, data[0].unsafeAddr) != 1:
...
proc writeJPG*(w, h, comp: int; data: openarray[byte]; quality: int): seq[byte] =
  ...
  if stbi_write_jpg_to_func(streamWriteData, buffer.addr, w.cint, h.cint,
      comp.cint, data[0].unsafeAddr, quality.cint) != 1:
